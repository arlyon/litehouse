"use client";
import { CheckIcon, DownloadIcon, CopyIcon } from "lucide-react";
...
export const CodeBlock = forwardRef<HTMLElement, CodeBlockProps>(
  (
    { title, allowCopy = true, allowDL = false, icon, className, children, ...props },
    ref,
  ) => {
        ...
        {title ? (
          ...
          <>
            ...
            <CopyButton
              ...
              onCopy={onCopy}
            />}
            {allowDL && <SaveButton className="absolute right-2 top-2 z-[2] backdrop-blur-sm" onSave={onSave} />}
          </>
        )}
...
function SaveButton({ className, onSave, ...props }: {className?: string, onSave: () => void}) {
